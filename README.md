...
![engine](https://img.shields.io/badge/engine-Unity%206%20LTS-black)
...
| **Generic orchestrators** — CrewAI, AutoGen, AgentBuilder | Built for office workflows; no game-dev opinion, no Unity, no asset pipeline |
...
```bash
git clone <this-repo> studio
cd studio
./core/scripts/observer-start.sh         # http://localhost:7777
./core/scripts/new-game.sh brave-bunny --template action-roguelite
# edit games/brave-bunny/GAME.md, then:
claude --dangerously-skip-permissions    # in the repo root
# inside Claude Code:  /phase-status
```

You'll need `claude` (Claude Code CLI), `tmux`, `python3 ≥ 3.11`, and — eventually, for engine work — Unity 6 LTS with the iOS build module.
